		...
		<WhiteContainer>
			<form onSubmit={handleSubmit} className="flex flex-col gap-4">
				...
				<div className="flex flex-col gap-1">
					...
					/>
					{titleError && (
						<p className="text-sm text-red-600">The log must have a title</p>
					)}
				</div>
				<div className="flex flex-col gap-1">
					...
					<textarea
						...
						required
						onBlur={() =>
							body === "" ? setBodyError(true) : setBodyError(false)
						}
						onChange={(e) => setBody(e.target.value)}
						className="border border-gray-detail rounded-lg px-2 py-1 h-[250px] resize-none md:h-[300px]"
					/>
					{bodyError && (
						<p className="text-sm text-red-600">The log must have a body</p>
					)}
				</div>
				<button
					type="submit"
